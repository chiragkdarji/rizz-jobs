...
export const metadata: Metadata = {
  title: "IPL 2025 Schedule — All Matches | Rizz Jobs",
  description: "Complete IPL 2025 match schedule with dates, venues, and results.",
};
...
export default async function SchedulePage() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const matchesList = data?.schedule?.matchScheduleMap ?? [];
      allMatches = matchesList
        .flatMap((m: { scheduleAdWrapper?: { matchScheduleList?: { matchInfo?: MatchInfo[] }[] } }) =>
          m.scheduleAdWrapper?.matchScheduleList?.flatMap((sl) => sl.matchInfo ?? []) ?? []
        );
    }
  ...
  for (const m of allMatches) {
    const d = new Date(parseInt(m.startDate)).toLocaleDateString("en-IN", { weekday: "long", day: "numeric", month: "long" });
              ...
              return (
                <Link key={m.matchId} href={`/ipl/match/${m.matchId}`}>
                  <div
                    ...
                    style={{
                      background: "#061624",
                      border: `1px solid ${isLive ? "#FF5A1F" : "#0E2235"}`,
                    }}
                  >
                    ...
                    <div className="flex items-center gap-2 flex-1">
                      <IplTeamBadge shortName={m.team1.teamSName} bg={t1c.bg} color={t1c.color} size="sm" />
                      <span style={{ color: "#6B86A0" }}>vs</span>
                      <IplTeamBadge shortName={m.team2.teamSName} bg={t2c.bg} color={t2c.color} size="sm" />
                    ...
                    <span
                      ...
                      style={{ color: isLive ? "#FF5A1F" : isDone ? "#22C55E" : "#6B86A0" }}
                    >
